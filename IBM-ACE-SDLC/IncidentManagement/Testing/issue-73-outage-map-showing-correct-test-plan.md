# Test Case Report -- Issue #73 -- Outage map are not showing correct time
**Artifact:** `issue-73-outage-map-showing-correct-test-plan.md` | **Branch:** `work/issue-73-bob-sdlc` | **Generated:** 2026-07-24
...
## Executive Summary
This test case report documents all verification activities performed for Issue #73.
...
- Fix resolves the reported behaviour: Outage map are not showing correct time
...
| TC-010 | Output of Main consistent with fix: Outage map are not showing correct time | Fix behaviour confirmed    | CR-001 | PASS |
...
|-----|----------|-------|-----------------|--------|
| TC-200 | Fix resolves: Outage map are not showing correct time | 1. Reproduce original defect condition 2. Apply fix 3. Verify corrected behaviour | Defect no longer reproducible | PASS |
...
|-----|-----------|---------------------|--------|
| TC-400 | Fix resolves the reported behaviour: Outage map are not showing correct time | Verified against fix in issue-73-outage-map-showing-correct-fix.esql | PASS |
| TC-401 | No hardcoded values in ESQL -- all configurable (CR-011 PASS) | Verified against fix in issue-73-outage-map-showing-correct-fix.esql | PASS |
| TC-402 | Code coverage >= 80% (QR-041 PASS) | Verified against fix in issue-73-outage-map-showing-correct-fix.esql | PASS |
| TC-403 | Performance overhead < 5ms per call (DR-014 PASS) | Verified against fix in issue-73-outage-map-showing-correct-fix.esql | PASS |
| TC-404 | All existing unit tests continue to pass | Verified against fix in issue-73-outage-map-showing-correct-fix.esql | PASS |
| TC-405 | Error handling present on all new code paths (CR-009 PASS) | Verified against fix in issue-73-outage-map-showing-correct-fix.esql | PASS |
