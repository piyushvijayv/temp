# Test Case Report -- Issue #74 -- Outage map are not coming properly for NPS
**Artifact:** `issue-74-outage-map-coming-properly-test-plan.md` | **Branch:** `work/issue-74-bob-sdlc` | **Generated:** 2026-07-24
...
## Executive Summary
This test case report documents all verification activities performed for Issue #74.
...
- Fix resolves the reported behaviour: Outage map are not coming properly for NPS
...
| TC-010 | Output of Main consistent with fix: Outage map are not coming properly for NPS | Fix behaviour confirmed    | CR-001 | PASS |
...
|-----|----------|-------|-----------------|--------|
| TC-200 | Fix resolves: Outage map are not coming properly for NPS | 1. Reproduce original defect condition 2. Apply fix 3. Verify corrected behaviour | Defect no longer reproducible | PASS |
...
|-----|-----------|---------------------|--------|
| TC-400 | Fix resolves the reported behaviour: Outage map are not coming properly for NPS | Verified against fix in issue-74-outage-map-coming-properly-fix.esql | PASS |
| TC-401 | No hardcoded values in ESQL -- all configurable (CR-011 PASS) | Verified against fix in issue-74-outage-map-coming-properly-fix.esql | PASS |
| TC-402 | Code coverage >= 80% (QR-041 PASS) | Verified against fix in issue-74-outage-map-coming-properly-fix.esql | PASS |
| TC-403 | Performance overhead < 5ms per call (DR-014 PASS) | Verified against fix in issue-74-outage-map-coming-properly-fix.esql | PASS |
| TC-404 | All existing unit tests continue to pass | Verified against fix in issue-74-outage-map-coming-properly-fix.esql | PASS |
| TC-405 | Error handling present on all new code paths (CR-009 PASS) | Verified against fix in issue-74-outage-map-coming-properly-fix.esql | PASS |
